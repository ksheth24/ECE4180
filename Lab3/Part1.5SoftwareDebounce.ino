...
void IRAM_ATTR navISR(void* arg) {
  ...
  if ((now - lastUpTime > 200) && pin == buttonUp) {
    color1 = (color1 + 1) % 6;
    lastUpTime = millis();
    currentColor = color1;
  } else if ((now - lastDownTime > 200) && pin == buttonDown) {
    color2 = (color2 + 1) % 6;
    lastDownTime = millis();
    currentColor = color2;
  } else if ((now - lastLeftTime > 200) && pin == buttonLeft) {
    color3 = (color3 + 1) % 6;
    lastLeftTime = millis();
    currentColor = color3;
  } else {
    color4 = ((now - lastRightTime > 200) && color4 + 1) % 6;
